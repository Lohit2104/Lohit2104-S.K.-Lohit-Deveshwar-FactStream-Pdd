...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($response === false) {
        echo "<script>
                alert('Failed to connect to the validation server.');
                window.location.href='upload.php';
              </script>";
        exit();
    }
        ...
        if (isset($innerJson['fact']) && $innerJson['fact'] === true) {
            ...
            if ($stmt->execute()) {
                echo "<script>
                        ...
                        window.location.href='adduploads.php';
                      </script>";
            } else {
                echo "<script>
            ...
            // Fact is false
            echo "<script>
                    alert('The given content is incorrect. Please verify and try again.');
                    window.location.href='upload.php';
                  </script>";
        }
    } else {
        echo "<script>
                alert('Invalid response from validation server.');
                window.location.href='upload.php';
              </script>";
    }
} else {
